...
class Activity(db.Model):
    ...
    last_update_desc: Mapped[str] = mapped_column(String(32), unique=True, nullable=False)
    creation_date: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    last_update_date: Mapped[datetime] = mapped_column(DateTime(timezone=True), onupdate=func.now())
...
def inicialize_activity_state_db():
    states = ["Criado", "Atualizado", "Excluído"]
    for state in states:
